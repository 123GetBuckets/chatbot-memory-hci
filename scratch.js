...
const systemMessage = {
role: "system",
content: "Given the following messages, create a summary.",
};

const chatIndex = chats.findIndex(chat => chat.id === chatId);
const messages = chats[chatIndex].messages;

const messageList = [systemMessage, ...selected
.filter(msg => msg.visible)
.map(msg => ({
role: msg.role,
content: msg.content
...
<button title='Summarize' onClick={() => handleSummarize(chat.id)} className='input-button'><StackIcon size={16} /></button>
<textarea
